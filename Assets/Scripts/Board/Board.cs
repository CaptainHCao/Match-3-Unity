...
public class Board
{
    ...
    // a reusable list
    private readonly List<NormalItem.eNormalType> _types = new List<NormalItem.eNormalType>();
    internal void Fill()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                item.SetType(Utils.GetRandomNormalTypeExcept(_types.ToArray()));
                item.SetView();
    ...
    internal void FillGapsWithNewItems()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Item item = m_cells[x, y].Item;
                if (item is NormalItem normal)
                {
                    // if type not in the dictionary yet, add it.
                    if (!typeCounts.ContainsKey(normal.ItemType)) typeCounts[normal.ItemType] = 0;
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                // Get surrounding types to avoid
                ...
                {
                    if (neighbour?.Item is NormalItem nitem) _types.Add(nitem.ItemType);
                }
                
                var allTypes = Enum.GetValues(typeof(NormalItem.eNormalType)).
                    Cast<NormalItem.eNormalType>().ToList();

                // Get a list of types that is not in _types list
                var validTypes = allTypes.Except(_types).ToList();
                ...
                // Get the least appeared type
                var selectedType = validTypes.FirstOrDefault();
                ...
                // Update type count at last
                if (!typeCounts.ContainsKey(selectedType)) typeCounts[selectedType] = 0;
